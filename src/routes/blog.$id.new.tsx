...
export const Route = createFileRoute("/blog/$id/new")({
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <p className="text-lg font-semibold text-teal">Loading blog...</p>
      </div>
    );
  }
  ...
  if (errorMessage) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
        <h2 className="text-3xl font-bold text-red-500">Error loading blog</h2>

        <p className="mt-3 text-muted-foreground">{errorMessage}</p>

        <Link to="/blog" className="mt-6 rounded-xl bg-teal px-6 py-3 font-semibold text-white">
          Back to Blogs
        </Link>
      </div>
    );
  ...
  if (!blog) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
        ...
        <p className="mt-3 text-muted-foreground">The blog you are looking for does not exist.</p>
        <Link to="/blog" className="mt-6 rounded-xl bg-teal px-6 py-3 font-semibold text-white">
          Back to Blogs
        </Link>
      </div>
    ...
    <div className="bg-white py-16">
      <div className="container mx-auto max-w-4xl px-6">
        ...
        <div className="mt-8">
          ...
          <h1 className="mt-5 text-4xl font-bold leading-tight text-gray-900">{blog.title}</h1>

          {blog.excerpt && (
            <p className="mt-4 text-lg italic text-gray-500">{blog.excerpt}</p>
          )}
